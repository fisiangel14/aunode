...
class Control():
    ...
    def _prerun_hook(self):
        if self.need_prerun_hook is True:
            ...
            if not hook_result:
                self._update(text_error=f'Control execution stopped because "rapo_prerun_control_hook" function evaluated as False (code={hook_code})')
                return False
        return True
class Parser():
...
class Executor():
    ...
    def prerun_hook(self):
        ...
        try:
            ...
            if result_code > 0:
                logger.debug(f'{self.c} Hook function evaluated True ({result_code})')
                return True, result_code
            else:
                logger.debug(f'{self.c} Hook function evaluated False ({result_code})')
